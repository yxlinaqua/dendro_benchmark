...
filepath1= dirpath+r'W49N_N_trim.fits'
...
##############write fits file##################
data=fits.getdata('/Users/yuxinlin/final_results_s/W49N_N_trim.fits')
...
#plt.imshow(data/mask, origin='lower',interpolation='nearest')
...
plt.show()
import os
outpath1=r'/Users/yuxinlin/final_results_s/W49N_N_erosion.fits'
...
fits.setval(outpath1,'BUNIT',value='Jy/pixel')
# -*- coding: utf-8 -*-
...
d = Dendrogram.compute(f,min_value=7.0e21,min_delta=6.0e20/2.35*5.,min_npix=7.0)#, is_independent = custom_independent)
...
d.save_to('W49N_my_dendrogram_erosion.fits')
...
#ax1.imshow((hdulist1[0].data), origin='lower', interpolation='nearest',
          #cmap=plt.cm.Blues, vmin=10**21.7,vmax=1e23)
#for leaf in d.leaves:
#        p3.plot_contour(ax1, structure=leaf, lw=3, colors='red')
#
#mask_ = fits.PrimaryHDU(~mask.astype('short'), hdulist1[0].header)
#ax2 = fig.add_subplot(1,3,2)
##ax2.hlines(3*10**22.09, *ax2.get_xlim(), color='b', linestyle='--')
#p3.plot_tree(ax2, color='black')
NH2_mean = []
...
for i,leaf in enumerate(d.leaves):
        if not (NH2_mean[i] < 10.0 *6.0e20/2.35*5. ):#and eff_radius[i]**2 > 0.15):# and not(NH2_mean[i] < 2.1e22 and eff_radius[i]**2 <0.08):
           p3.plot_contour(ax1, structure=leaf, lw=3, colors='red')
...
ax2.hlines(10*4.5e20/2.35*5., *ax2.get_xlim(), color='b', linestyle=':')
...
#inset_axes = zoomed_inset_axes(ax2,
                              # 0.5, # zoom = 0.5
                              # loc=1)
axins = zoomed_inset_axes(ax2, 3.5, loc=2) # zoom = 6
p3.plot_tree(axins, color='black')
for i,leaf in enumerate(d.leaves):
        if not (NH2_mean[i] < 10*6.0e20/2.35*5. ):#and eff_radius[i]**2 > 0.15):# and not(NH2_mean[i] < 2.1e22 and eff_radius[i]**2 <0.08):
          # p3.plot_contour(ax1, structure=leaf, lw=3, colors='red')
...
x1,x2,y1, y2 = 10,20,1e21, 0.5e23
axins.set_xlim(x1, x2)
axins.set_ylim(y1, y2)
axins.hlines(10*6.0e20/2.35*5., *ax2.get_xlim(), color='b', linestyle=':')
#ax2.set_xticks([])
axins.set_yticks([])
...
################draw m-r for all structures, from Adam#################################
...
def dendroplot(axis=ax3, axname1='area_exact', axname2='flux',
        ...
        for leaves, color in zip(leaves_list,color_list):
            for i,leaf in enumerate(leaves):
              ...
              if not (NH2_mean[i] < 10*6.0e20/2.35*5. ):
               ...
               if obj is not None:
                ...
                #    if np.all(signs==1) or np.all(signs==-1):
                #        axis.plot(xax, yax, alpha=0.1, linewidth=5, zorder=0, color='g')
ax3.plot(eff_radius_leaf[NH2_mean> 10*6.0e20/2.35*5. ], mass_leaf[NH2_mean > 10*6.0e20/2.35*5.] ,'s',color='red',label='mass',markeredgecolor='none',alpha=0.5) 
ax3.plot(eff_radius_leaf[NH2_mean > 10*6.0e20/2.35*5. ], mass_corrected_leaf[NH2_mean> 10*6.0e20/2.35*5. ],'s',color='purple',label='corrected mass',markeredgecolor='none',alpha=0.5) 
...
(ar,br)=polyfit(np.log10(eff_radius_leaf[NH2_mean> 10*6.0e20/2.35*5. ]),np.log10(mass_leaf[NH2_mean> 10*6.0e20/2.35*5. ]),1)
...
##linear regression###
from scipy import linspace, polyval, polyfit, sqrt, stats, randn
(ar1,br1)=polyfit(np.log10(eff_radius_leaf[NH2_mean> 10*6.0e20/2.35*5. ]),np.log10(mass_corrected_leaf[NH2_mean> 10*6.0e20/2.35*5. ]),1)
xr1=polyval([ar1,br1],np.log10(r.value))

ax3.plot(r,np.power(10,xr1),'--',color='purple')
print ar1,ar,br1,br
##################save the plot########################################
fig.savefig(outpath+'W49N_dendro_mass_radius1_erosion_prune.pdf'
            ,bbox_inches='tight')
